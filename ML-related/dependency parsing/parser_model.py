...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        tmp = []
        for i in w:
            each_batch_embeddings = []
            for j in i:
                each_batch_embeddings+=self.embeddings[j]
            tmp.append(each_batch_embeddings)
    ...
    def forward(self, w):
        ...
        h = nn.functional.relu(x@self.embed_to_hidden_weight+self.embed_to_hidden_bias)
        l = self.dropout1(h)@self.hidden_to_logits_weight+self.hidden_to_logits_bias
        return l
